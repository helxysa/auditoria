...
import type { AuditoriaPorTipo } from "@/types/dashboard";

interface ChartBarAuditoriasTipoProps {
  data: AuditoriaPorTipo[];
  title?: string;
  description?: string;
  showFooter?: boolean;
}
...
export function ChartBarAuditoriasTipo({
  data,
  title = "Auditorias por Tipo",
  description = "Distribuição de auditorias agrupadas por tipo",
  showFooter = true,
}: ChartBarAuditoriasTipoProps) {
        ...
        <CardFooter className="flex-col items-start gap-2 text-sm border-t pt-4">
          {maxTipo && (
            <div className="flex items-center gap-2 leading-none font-medium text-gray-700">
              ...
              <span>Tipo com mais auditorias: <span className="text-blue-600">{maxTipo.tipo}</span> ({maxTipo.quantidade})</span>
            </div>
          )}
          <div className="text-gray-500 leading-none">
            Total de {totalCount} auditorias distribuídas em {data.length} tipos
          </div>
        </CardFooter>
      )}
    </Card>
